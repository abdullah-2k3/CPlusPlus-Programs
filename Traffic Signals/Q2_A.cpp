...
string get_departure_time(string arrival_time, int signalTime) {
  ...
  if (new_hour < 10) _time += '0';

  _time += to_string(new_hour);
  _time += ':';

  if (new_min < 10) _time += '0';

  _time += to_string(new_min);
  _time += ':';

  if (new_sec < 10) _time += '0';
...
class Car {
 ...
 public:
  ...
  void set_departure_time(string time) { departure_time = time; }

  string get_departure_time() { return departure_time; }

  string get_arrival_time() { return arrival_time; }
...
class TrafficSignal {
 ...
 public:
  ...
  TrafficSignal(string direction) { this->direction = direction; }

  void set_direction(string direction) { this->direction = direction; }

  Queue<Car> get_car_queue() { return cars; }
  ...
  void runSignal(int signalTime) {
    while (signalTime > 0 && !cars.isEmpty()) {
      ...
      cout << "Waiting Time for this Car: "
           << get_time_diff(arrival_time, last_car_departure_time) << endl
           << endl;
 ...
 public:
  ...
  void runSimulation() {
    for (int i = 0; i < 4; i++) {
      ...
      cout << "Time for this signal: " << signalTime / 8 << " seconds" << endl
           << endl;
      cout << "--------------------------------------------\n\n";
...
int main() {
  ...
  while (true) junction1.runSimulation();
